...
while cv2.getWindowProperty('Video', 0) >= 0:
    ...
    for (x, y, w, h) in faces:
        ...
        sub_img2 = frame[int(y) + int(h/2):int(y+h),int(x):int(x+w),:] #only analize half of face for mouth
        mouth = apply_Haar_filter(sub_img2, haar_mouth, 1.3 , 10, 10)
        for (x2, y2, w2, h2) in mouth:
            cv2.rectangle(frame, (int(x+x2), int(y+h/2+y2)), (int(x + int(x2+w2)), int(y+h/2+y2+h2)), GREEN, 2) #green
